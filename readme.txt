...
3. 目录结构如下
   ...
   /ShowQImage
     ...
     ├ Include  ： 头文件
     │   ├ Media       : 转码库的头文件
     │   ├ GenICam     : SDK库的头文件。请配合所用SDK版本里自带的头文件。(SDK安装目录下\Development\Include中)
     │   ├ Infra       ：SDK库的头文件。请配合所用SDK版本里自带的头文件。(SDK安装目录下\Development\Include中)
     │   └ Memory      ：SDK库的头文件。请配合所用SDK版本里自带的头文件。(SDK安装目录下\Development\Include中)
     │
     ├ Depends  ：  编译时依赖库
     │   ├ ImageConvert.lib:  转码库
